function [df_Wv,df_Wo,df_W,df_WO] = backpropAll(thisTree,W,WO,Wo,params,deltaUp_vec,deltaUp_op,thisNode,numWordsInBatch)
...
df_Wv = spalloc(wsz,numWordsInBatch,(numWords+2)*wsz); % plus 2 for possible padding

% df_Wo = spalloc(wsz+2*r*wsz,numWordsInBatch,(numWords+params.NN*2)*(wsz+2*r*wsz));
df_Wo = spalloc(wsz+2*r*wsz,numWordsInBatch,(numWords)*(wsz+2*r*wsz));
...
%         
        ...
        [df_Wv_new,df_Wo_new,df_W_new,df_WO_new] = backpropAll(thisTree,W,WO,Wo,params,deltaDown_vec{c},deltaDown_op{c},kids(c),numWordsInBatch);
