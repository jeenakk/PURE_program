...
% % FID = fopen('sentences_5000_notSeparated.txt', 'w');
% FID = fopen('sentences_1600_notSeparated.txt', 'w');
% for j = 1:size(textAll2, 2)
%     s = textAll2{j}; 
%     fprintf(FID, '%s\n', s{1});
%     s = hypothesisAll2{j}; 
%     fprintf(FID, '%s\n',  s{1});
...
% FID = fopen('sentences_5000_hypothesis.txt', 'w');
FID = fopen('sentences_1600_hypothesistxt', 'w');
for j = 1:size(textAll2, 2)
    s = hypothesisAll2{j}; 
    fprintf(FID, '%s\n',  s{1});
end
...
% FID = fopen('sentences_5000_texts.txt', 'w');
FID = fopen('sentences_1600_texts.txt', 'w');
