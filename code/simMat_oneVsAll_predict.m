...
% compute features
[Trees3 Trees4] = getInstancesParsed_onlyTest(test.allSNum,test.allSKids,test.allSTree,theta,We2,params);
...
[dataTest]= preProData_release_onlyTest(data.test,params2);

save(outFile ,'dataTest','params');
...
%% Real test 
pairId_test = load(['../EntailmentData/PairId_test.txt']); 


% similarity 
load best_NN_1:36am_march_22nd; 
input_test = [dataTest.X; dataTest.otherFeat]'; 
similarityPrediction = 4*sim(minNet, input_test')+1; 
writeTheSemEVALOutput('sample1.txt', similarityPrediction, [],  pairId_test) 

% entailment 
load best_entailment_acc=7164_March_23rd_2:34am; 
input_test = [dataTest.X; dataTest.otherFeat]';
labels_0_test = ones(size(input_test, 1), 1); 
 [predicted_labels_0,acc_0,dv_0] = predict(labels_0_test, sparse(input_test),Temp1,'-b 1');
 [predicted_labels_Neg1,acc_Neg1,dv_Neg1] = predict(labels_0_test, sparse(input_test),Temp2,'-b 1');
 [predicted_labels_1,acc_1,dv_1] = predict(labels_0_test, sparse(input_test),Temp3,'-b 1');
 [MaxVal,I] = max([(dv_Neg1(:,2))';(dv_0(:,1))';(dv_1(:,2))']);
 labels_all = I-2;
 

writeTheSemEVALOutput('asjai_5.txt', similarityPrediction, labels_all,  pairId_test) 
