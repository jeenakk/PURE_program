...
if(MC160)
    inputFile = ['../EntailmentData/' dataFolder  'unsupervised_pased_1600.txt'];
    sentencesText = textread(['../EntailmentData/' dataFolder 'unsupervised_sentences_1600.txt'], '%s', 'delimiter', '\n');
    parseTreeFile = 'unsupervised_sentences_1600_all';
else
    % TODO
end
...
if(MC160)
    inputFile = ['../EntailmentData/' dataFolder  'parsed_1600.txt'];
    sentencesHyp = textread(['../EntailmentData/' dataFolder 'sentences_1600.txt'], '%s', 'delimiter', '\n');
    parseTreeFileHyp = 'unsupervised_sentences_1600_hyp';
else
    % TODO
end
% convertStanfordParserTrees
...
clearvars -except dataFolder MC160 sentencesText parseTreeFile parseTreeFileHyp   

if(MC160)
    load(['../EntailmentData/' dataFolder  parseTreeFile]);
    load(['../EntailmentData/' dataFolder  'textSizes_1600.txt']);
    textsProcessed = test; 
    load(['../EntailmentData/' dataFolder  parseTreeFileHyp]);
    sentences_hyp = textread([ '../EntailmentData/' dataFolder 'sentences_1600.txt'], '%s', 'delimiter', '\n');
    hypsProcessed = test; 
...
iterAll= 1;
iterAllHyp = 1; 
for i = 1:size(textSizes_1600,1)
%     sentences_hypTmp = sentences_hyp(iterAllHyp : iterAllHyp + textSizes_1600(i) - 1 ); 
    
    % positive
    training_labels = [training_labels  ones(1, textSizes_1600(i)-1)];
%     sentencesTmpP = sentencesText(iterAll : iterAll + textSizes_1600(i) - 1 ); 
    for j = 0:textSizes_1600(i) - 2
        sentencesAll{end+1} = sentencesText{iterAll + j}; 
        sentencesAll{end+1} = sentences_hyp{iterAllHyp + j+1}; 

        train.allSNum(end+1) = textsProcessed.allSNum(iterAll + j );
        train.allSStr(end+1) = textsProcessed.allSStr( iterAll + j );
        train.allSTree(end+1) = textsProcessed.allSTree( iterAll + j );
        train.allSKids(end+1) = textsProcessed.allSKids( iterAll + j );
        
        train.allSNum(end+1) = hypsProcessed .allSNum( iterAllHyp +  j+1 );
        train.allSStr(end+1) = hypsProcessed .allSStr( iterAllHyp + j+1 );
        train.allSTree(end+1) = hypsProcessed .allSTree( iterAllHyp + j+1 );
        train.allSKids(end+1) = hypsProcessed .allSKids( iterAllHyp + j+1 );
    end 
    iterAll = iterAll + textSizes_1600(i); 
    
    % negative
%     sentencesTmpN = sentencesText(iterAll : iterAll + textSizes_1600(i) - 1 );     
    training_labels = [training_labels  zeros(1, textSizes_1600(i)-1)];
    for k = 2:textSizes_1600(i)
        sentencesAll{end+1} = sentencesText{iterAll + k - 1};  
        sentencesAll{end+1} = sentences_hyp{iterAllHyp + textSizes_1600(i) - k};  % jth in the reversed array  

        train.allSNum(end+1) = textsProcessed.allSNum( iterAll + k-1 );
        train.allSStr(end+1) = textsProcessed.allSStr( iterAll + k-1 );
        train.allSTree(end+1) = textsProcessed.allSTree( iterAll + k-1 );
        train.allSKids(end+1) = textsProcessed.allSKids( iterAll + k-1 );
        
        train.allSNum(end+1) = hypsProcessed .allSNum( iterAllHyp + textSizes_1600(i) - k );
        train.allSStr(end+1) = hypsProcessed .allSStr( iterAllHyp + textSizes_1600(i) - k );
        train.allSTree(end+1) = hypsProcessed .allSTree( iterAllHyp + textSizes_1600(i) - k );
        train.allSKids(end+1) = hypsProcessed .allSKids( iterAllHyp + textSizes_1600(i) - k );
    end 
    iterAll = iterAll + textSizes_1600(i); 
    iterAllHyp= iterAllHyp + textSizes_1600(i);    
%     iterAll
end

training_sentences = sentencesAll; 
...
% compute features
[Trees1 Trees2 ] = getInstancesParsed_onlyTestOrTrain(train.allSNum,train.allSKids,train.allSTree,theta,We2,params);
...
save(outFile, 'dataFullTrain','params');
...
c = 0.005;
e = 0.01;
model_0 = train(dataFullTrain.labels', sparse([dataFullTrain.X; dataFullTrain.otherFeat]'), ['-s 0 -c ' num2str(c)]);
model_0 = train(dataFullTrain.labels', sparse([dataFullTrain.X; dataFullTrain.otherFeat]'), ['-s 0 ' ]);

[predicted_labels_0,acc_0,dv_0] = predict(dataFullTrain.labels', sparse([dataFullTrain.X; dataFullTrain.otherFeat]'),model_0);

model_1 = train(dataFullTrain.labels', sparse([dataFullTrain.X; dataFullTrain.otherFeat]'), model_0, ['-s 0 -c ' num2str(c)]);
